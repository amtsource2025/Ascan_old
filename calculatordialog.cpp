...
<<<<<<< HEAD
    ...
    QSqlQuery q(db);
    q.prepare("SELECT id, iol, company FROM lens ORDER BY iol ASC");

    if (!q.exec())
    {
       // qDebug() << "populateCombo Error:" << q.lastError().text();
        cb->blockSignals(false);
        return;
    }
    ...
    while (q.next())
    {
        ...
        QString label;
        if (company.trimmed().isEmpty())
            label = iol;
        else
            label = QString("%1 (%2)").arg(iol, company);

        // Store the database ID in UserRole
        cb->addItem(label, id);
...
<<<<<<< HEAD
    ...
    switch (panelNum)
    {
    case 1: cb = ui->comboBox_lensOne;   break;
    case 2: cb = ui->comboBox_lensTwo;   break;
    case 3: cb = ui->comboBox_lensThree; break;
    default: return;
    }
    ...
    // Get database ID stored in combo box
    int lensId = cb->itemData(comboIndex).toInt();

    qDebug() << "Selected Index :" << comboIndex;
    qDebug() << "Lens ID        :" << lensId;
    ...
    QSqlQuery q(db);
    q.prepare(
        "SELECT asrkt, asrkii, acd, sf, a0, a1, a2 "
        "FROM lens "
        "WHERE id = :id");

    q.bindValue(":id", lensId);
...
>>>>>>> c45e4e3d7db5a6fc95b663ed077b6e12fe2a2556
    ...
    double sf     = q.value(3).toDouble();
<<<<<<< HEAD
    double a0     = q.value(4).toDouble();
    double a1     = q.value(5).toDouble();
    double a2     = q.value(6).toDouble();



    auto setValue = [](QLineEdit *le, double value)
    {
        if (le)
            le->setText(QString::number(value, 'f', 2));
    };

    if (panelNum == 1)
    {
        setValue(ui->lineEdit_aconst1,  asrkii);
        setValue(ui->lineEdit_aconst12, asrkt);

        setValue(ui->lineEdit_acd1,     acd);
        setValue(ui->lineEdit_acd12,    acd);

        setValue(ui->lineEdit_sf1,      sf);
        setValue(ui->lineEdit_sf12,     sf);

        setValue(ui->lineEdit_A0,       a0);
        setValue(ui->lineEdit_A02,      a1);

        // If you have a third Haigis field
        // setValue(ui->lineEdit_A03, a2);
    }
    else if (panelNum == 2)
    {
        setValue(ui->lineEdit_aconst2,  asrkii);
        setValue(ui->lineEdit_aconst22, asrkt);

        setValue(ui->lineEdit_acd2,     acd);
        setValue(ui->lineEdit_acd22,    acd);

        setValue(ui->lineEdit_sf2,      sf);
        setValue(ui->lineEdit_sf22,     sf);

        setValue(ui->lineEdit_A1,       a0);
        setValue(ui->lineEdit_A12,      a1);

        // setValue(ui->lineEdit_A13, a2);
    }
    else
    {
        setValue(ui->lineEdit_aconst3,  asrkii);
        setValue(ui->lineEdit_aconst32, asrkt);

        setValue(ui->lineEdit_acd3,     acd);
        setValue(ui->lineEdit_acd32,    acd);

        setValue(ui->lineEdit_sf3,      sf);
        setValue(ui->lineEdit_sf32,     sf);

        setValue(ui->lineEdit_A2,       a0);
        setValue(ui->lineEdit_A22,      a1);

        // setValue(ui->lineEdit_A23, a2);
    }

    qDebug() << "Displayed ASRKT :" << ui->lineEdit_aconst12->text();
=======
    double a0     = q.value(4).toDouble();  // Haigis a0
    double a1     = q.value(5).toDouble();  // Haigis a1
    double a2     = q.value(6).toDouble();  // Haigis a2 — FIX: was never read before
...
{
<<<<<<< HEAD
    bool showSRKII = (m_formula == IolFormula::SRKII);
    bool showSRKT  = (m_formula == IolFormula::SRKT);
    bool showSF    = (m_formula == IolFormula::Holladay);
    bool showAcd   = (m_formula == IolFormula::HofferQ);
    bool showHaigis = (m_formula == IolFormula::Haigis);

    // ================= Panel 1 =================

    if (ui->lbl_aconst1) ui->lbl_aconst1->setVisible(showSRKII || showSRKT);
    if (ui->lbl_sf1)      ui->lbl_sf1->setVisible(showSF);
    if (ui->lbl_acd1)     ui->lbl_acd1->setVisible(showAcd);
    if (ui->lbl_a0)       ui->lbl_a0->setVisible(showHaigis);

    if (ui->lineEdit_aconst1)   ui->lineEdit_aconst1->setVisible(showSRKII);
    if (ui->lineEdit_aconst12)  ui->lineEdit_aconst12->setVisible(showSRKT);
    ...
    if (ui->lineEdit_A0)        ui->lineEdit_A0->setVisible(showHaigis);
    if (ui->lineEdit_A02)       ui->lineEdit_A02->setVisible(showHaigis);

    // ================= Panel 2 =================

    if (ui->lbl_aconst2) ui->lbl_aconst2->setVisible(showSRKII || showSRKT);
    if (ui->lbl_sf2)      ui->lbl_sf2->setVisible(showSF);
    if (ui->lbl_acd2)     ui->lbl_acd2->setVisible(showAcd);

    if (ui->lineEdit_aconst2)   ui->lineEdit_aconst2->setVisible(showSRKII);
    if (ui->lineEdit_aconst22)  ui->lineEdit_aconst22->setVisible(showSRKT);
    ...
    if (ui->lineEdit_A1)        ui->lineEdit_A1->setVisible(showHaigis);
    if (ui->lineEdit_A12)       ui->lineEdit_A12->setVisible(showHaigis);

    // ================= Panel 3 =================

    if (ui->lbl_aconst3) ui->lbl_aconst3->setVisible(showSRKII || showSRKT);
    if (ui->lbl_sf3)      ui->lbl_sf3->setVisible(showSF);
    if (ui->lbl_acd3)     ui->lbl_acd3->setVisible(showAcd);

    if (ui->lineEdit_aconst3)   ui->lineEdit_aconst3->setVisible(showSRKII);
    if (ui->lineEdit_aconst32)  ui->lineEdit_aconst32->setVisible(showSRKT);
    ...
    if (ui->lineEdit_A2)        ui->lineEdit_A2->setVisible(showHaigis);
    if (ui->lineEdit_A22)       ui->lineEdit_A22->setVisible(showHaigis);
=======
    bool showAconst = (m_formula == IolFormula::SRKII || m_formula == IolFormula::SRKT);
    bool showSF     = (m_formula == IolFormula::Holladay);
    bool showAcd    = (m_formula == IolFormula::HofferQ);
...
<<<<<<< HEAD
    ...
    if (p == 1)
    {
        if (m_formula == IolFormula::SRKT)
            c.aConst = dbl(ui->lineEdit_aconst12, 118.4);   // SRK/T
        else
            c.aConst = dbl(ui->lineEdit_aconst1, 118.4);    // SRK II
    ...
    {
        if (m_formula == IolFormula::SRKT)
            c.aConst = dbl(ui->lineEdit_aconst22, 118.4);
        else
            c.aConst = dbl(ui->lineEdit_aconst2, 118.4);
    ...
    {
        if (m_formula == IolFormula::SRKT)
            c.aConst = dbl(ui->lineEdit_aconst32, 118.4);
        else
            c.aConst = dbl(ui->lineEdit_aconst3, 118.4);
